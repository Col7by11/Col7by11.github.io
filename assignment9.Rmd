...
library(tidyverse)
library(lubridate)
df <- read_csv("C:\\Users\\student\\OneDrive - Bryant University\\Desktop\\R\\Col7by11.github.io\\WHO-COVID-19-global-data.csv")
df$month <- month(df$Date_reported)
df %>% 
  filter(Date_reported=='2022-10-12') %>% arrange(-Cumulative_deaths) %>% top_n(3, Cumulative_deaths)
```
...
df %>% 
  filter(Date_reported=='2022-10-12') %>% arrange(-death_per_cases) %>% top_n(3, death_per_cases)
```

4.Use `transition_reveal` to make a plot revealing the number of deaths per cases of the US, Italy and Mexico. 
```{r}
df %>% 
  filter(Country_code %in% c('US','MX','IT')) %>% 
