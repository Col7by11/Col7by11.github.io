...
library(tidyverse)
library(ggplot2)
df <- read_csv("C:\\Users\\student\\OneDrive - Bryant University\\Desktop\\R\\titanic.csv")
library(lubridate)
df <- df %>% 
...
- Scatter Plot
```{r}
df %>% ggplot()+geom_point(aes(x=Age, y=Fare, color=Sex)) 
```
...
- Smooth Plot
```{r}
df %>% ggplot()+geom_smooth(aes(x=Age, y=Fare, color= Sex))
```
...
```{r}
df %>% ggplot()+
  geom_bar(mapping=aes(x=Embarked, fill=Pclass), position = 'fill')
```
