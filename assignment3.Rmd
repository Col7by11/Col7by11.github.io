...
2. Show the names of the variables in the dataz
...
```{r}
library(dplyr)
df$death2 <- case_when(df$Deaths==0 ~ 'no_death',TRUE ~ 'has_death')
```
...
```{r}
df$death3 <- case_when(df$Deaths<846 ~ 'low-death', df$Deaths <12718 ~'mid_death', TRUE ~ 'high_death')
```
...
df2 = df[df$State=='Rhode Island',]
by(df$Cases,df$months, median)
```
...
- case_when
```{r}
df <- read.csv("C:/Users/student/OneDrive - Bryant University/Desktop/R/titanic.csv")
head(df)
str(df)
names(df)
...
df$Age2 <- case_when(df$Age<18 ~ 'Child',df$Age<64 ~ 'Adult',TRUE~'Elder')
library(lubridate)
df$Embarked2 <- case_when(df$Embarked %in% c('Q','S') ~ 'England', TRUE ~ 'France')
```
