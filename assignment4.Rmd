...
df$weekend <- case_when(df$weekday <6 ~ 'weekday', TRUE ~ 'weekend')
```
...
```{r}
df %>% group_by(month2) %>% summarise(mean(positiveIncrease, na.rm=TRUE))
```

- Calculate the median number of cases (`positiveIncrease`) by `weekend`
```{r}
df %>% group_by(weekend) %>% summarise(median(positiveIncrease, na.rm=TRUE))
```
...
```{r}
df %>% filter(month==10|month==11) %>% group_by(month) %>% summarise(positiveIncrease=median(positiveIncrease))
```

- Calculate the average number of death at different periods of a month (`month2` variable) in Fall 2020
```{r}
df %>% filter(month==9|month==10|month==11) %>% group_by(month2) %>% summarise(death=mean(death))
```
...
df %>% filter(month==6|month==7|month==8) %>% group_by(weekend) %>% summarise(hospitalized=mean(hospitalized))
df %>% filter(month==6|month==7|month==8) %>% group_by(weekdays) %>% summarise(hospitalized=mean(hospitalized))
```
...
df2 %>% filter(state=='Rhode Island') %>% group_by(weekday) %>% summarise(cases=median(cases))
library(lubridate)
df2$month <- month(df2$date, label=TRUE)
df2 %>% filter(date>='2021-01-01'&date<='2021-12-31') %>% 
  filter(state=='Rhode Island') %>% filter(month=='Jun'|month=='Jul'|month=="Aug"|month=='Sep') %>% group_by(month) %>%  summarise(cases=median(cases))
```
...
```{r}
df2 %>%  filter(weekday=='Saturday'|weekday=='Sunday') %>% group_by(date) %>% summarise(max_deaths=max(deaths)) %>% arrange(-max_deaths)
```
...
```{r}
df3 <- read.csv("C:/Users/student/OneDrive - Bryant University/Desktop/R/titanic.csv")
df3 %>% select(Age)
df3 %>% filter(Age==0)
df3 %>% mutate(SibSp = 'Sib or Sp')
df3 %>% summarise(mean_Age=mean(Age, na.rm=TRUE))
df3 %>% arrange(Age)
df3 %>% count(Age)
df3 %>% count(Age) %>% arrange(-n)
df3 %>% filter(Sex=='male') %>% count(Age) %>% arrange(-n)
df3 %>% group_by(Embarked) %>% summarise(mean_Age =mean(Age, na.rm=TRUE))
df3 %>% filter(Sex=='male') %>% group_by(Embarked) %>% summarise(mean_Age=mean(Age, na.rm=TRUE))
df3 %>% filter(Sex== 'male') %>% group_by(Embarked) %>% summarise(mean_Age=mean(Age, na.rm=TRUE)) %>% arrange(-mean_Age)
```
